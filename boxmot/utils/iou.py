...
if func in [iou_batch, giou_batch, diou_batch, ciou_batch]:
    if len(args) != 4 or not all(isinstance(arg, (list, np.ndarray)) for arg in args[:4]):
        raise ValueError("Invalid arguments for iou functions. Expected four bounding boxes.")
    return func(*args)
elif func is centroid_batch:
    if len(args) != 4 or not all(isinstance(arg, (list, np.ndarray)) for arg in args[:2]) or not all(isinstance(arg, int) for arg in args[2:4]):
