...
if func not in [iou_batch, giou_batch, diou_batch, ciou_batch, centroid_batch]:
    raise ValueError("Invalid function specified. Must be either 'iou_batch' or 'centroid_batch'.")

if func is iou_batch:
    ...
    if len(args) != 6 or not all(isinstance(arg, (list, np.ndarray)) for arg in args[:2]) or not all(isinstance(arg, (int)) for arg in args[2:]):
